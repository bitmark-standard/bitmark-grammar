...
class BitUtil {
  ...
  getcode(ctx, could_end_with_any = false) {
    if (!ctx.children) {
      ...
      let tmps = R_slice(ctx.start.start + bk, stop, this.source);
      ...
      if (this.source[start - 1] === '|') {
        ...
        if (0 < nlat && (nlat - start) < 8) {  // 8 is a hack value
          ...
          while (this.source[start - nloff] != '\n')
            nloff++;
          nloff--;   // skip \n
          let sr = R_slice(start - nloff, stop + nloff, this.source);
          return sr;
        }
        ...
        tmps = R_slice(start + nloff, stop + nloff + magic, this.source);
        let i = 0;
        while (tmps[i++].match(/[ \t\n\r]/))
          nloff++;
        if (nloff0 != nloff)
