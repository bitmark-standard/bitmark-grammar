...
es6: $(grammar_file)
	python ./tools/replace_parser_content.py $(grammar_dir)/_chatLexer.g4 $(common_lexer) $(build_dir)/chatLexer.g4 '<<<<<<common<<<<<<'
	python ./tools/replace_parser_content.py $(grammar_dir)/_chatParser.g4 $(common_parser) $(build_dir)/chatParser.g4 '<<<<<<common<<<<<<'
	...
	$(TYPESCRIPTCOMPILER) $(build_dir)/char*.ts -experimentalDecorators --strict true --module commonjs  --target es2015 -lib es2015 --moduleResolution Node --outDir  $(build_dir)
